...
export default function FleetReview({ onExit, backLabel = '◂ RETURN TO MAP', testMode = false }) {
  ...
  // Bastion Bulwark reads as one "+50 FLAGSHIP HULL" line, and so on) — shared
  // with the shipyard's hover intel, and built on the same aggregation the
  // battle applies, so the list can never drift from what actually deploys
  const statTotals = hasUpgrades ? upgradeStatLines(getUpgrades()) : []
